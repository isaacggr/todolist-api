...
function showPage(page) {
   ...
   const currentPage = [homePage, loginPage, registerPage, dashboardPage].find(
...
function init() {
   ...
   taskForm.addEventListener('submit', async function (e) {
      ...
      submitBtn.innerHTML =
         '<span class="material-icons loading-spinner">sync</span> Salvando...';
      ...
      try {
         const taskData = {
            ...
            startAt: formatDateToISO(
               document.getElementById('taskStartAt').value
            ),
         ...
         // Animação de sucesso
         submitBtn.innerHTML =
            '<span class="material-icons success-icon">check_circle</span> Salvo!';
   ...
   showAddTask.addEventListener('click', () => {
      ...
      document.getElementById('taskEndAt').value = formatDateTimeForInput(
...
async function handleLogin(e) {
   ...
   submitButton.innerHTML =
      '<span class="material-icons loading-spinner">sync</span> Entrando...';
...
async function handleRegister(e) {
   ...
   const originalText = submitButton.innerHTML;
   submitButton.innerHTML =
      '<span class="material-icons loading-spinner">sync</span> Cadastrando...';
   submitButton.disabled = true;
   ...
   try {
      ...
      showNotification(
         error.message || 'Usuário já existe ou ocorreu um erro',
         'error'
      );
...
async function openEditTaskForm(taskId) {
   try {
      ...
      document.getElementById('taskEndAt').value =
...
async function handleTaskAction(taskId, action) {
   ...
   try {
      ...
      const button = document.querySelector(
         `button[data-id="${taskId}"][class*="btn-${action}"]`
      );
      if (button) {
            ...
            if (newCompletedStatus) {
               ...
               if (!taskHeader.querySelector('.completed-badge')) {
                  ...
                  completedBadge.innerHTML =
                     '<span class="material-icons">check_circle</span> Concluído';
               ...
               const completedBadge =
                  taskCard.querySelector('.completed-badge');
               if (completedBadge) {
...
function createTaskElement(task) {
   ...
   const completeButtonTitle = task.completed
   ...
   taskCard.innerHTML = `
        <div class="task-header">
            ...
            <div class="task-priority priority-${
               task.priority
            }">${formatPriority(task.priority)}</div>
            ${completedBadgeHTML}
        </div>
        ...
        <div class="task-actions">
            <button class="btn-edit" data-id="${
               task.id
            }" title="Editar tarefa"><span class="material-icons">edit</span></button>
            <button class="btn-complete" data-id="${
               task.id
            }" title="${completeButtonTitle}"><span class="material-icons">${completeButtonIcon}</span></button>
            <button class="btn-delete" data-id="${
               task.id
            }" title="Excluir tarefa"><span class="material-icons">delete</span></button>
        </div>
    `;
